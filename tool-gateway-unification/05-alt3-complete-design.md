...
> Level: **complete design** — Alternative 3 (Unified Tool Gateway Service), Revision 2 post-review. Current architecture, **no bus**. The selected target design.
...
> Down-link: adversarial review record → [06-alt3-review.md](06-alt3-review.md).
>
> Taken **as the selected target design** (going straight to Alt 3, per [02-recommendation.md §2.2](02-recommendation.md)'s "if Alt 3 is committed, go straight to it" branch). Creates **one supervisor owning the tool's whole non-host external surface + tool coordination**, with the fab-qualified GEM/motion internals kept intact in-process. (Frameworks: the control unit is **.NET Framework 4.x** — `v4.8` via the shared `Camtek.CSharp.Common.Properties.props`; the egress gateway is `net7.0-windows` — the cross-CLR boundary the design keeps out of the control path.)
