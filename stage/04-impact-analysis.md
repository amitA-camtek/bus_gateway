...
| `Messaging\Camtek.Messaging.TestKit` / `.Tests` / `.Tap` | 14 contract-assertion groups (incl. 5b/5c/6b) + the gateway suite (G-1..G-9) + T-L load tests; fault injection; bus-tap CLI | A |
...
**Dependency notes (policy §0.4):** `System.Threading.Channels` is **already referenced by a net48 project** (`objects\DataCollector.Net`, v6.0.0, shipped in the binary drop both bitnesses) — not a new approval, just **pin/align to the shipped version** (FEA7-4); broker embed library only if the P0 build-vs-embed decision selects embed. **New Wave-0/P1a test deliverables (M-20/M-21):** the **GatewayHarness** + gateway suite (`ToolGateway.Tests` extended — FakeSink with settable reachability + typed scripted results, `CrashAt(GatewayStep)`, torn-append fixture, old-format `FailedMessages` migrator fixtures) is a named deliverable, same treatment record-replay got under R-TS-2 — an undefined suite cited as a gate is unenforceable.
...
| P1a | `Utilities\ToolGateway\ToolGateway.BL` + `.Endpoint` + `.Tests` | + `BusSource` (WAL-before-ack) + `CommandPublisher` (:5007); net7 → net8-era, **ships on .NET 10 LTS per §4.4** (R-OPS-6). **:5007 default-deny authz is a P1a exit criterion** (security work-stream, [05 §5.6](05-roadmap-and-risks.md)) | Medium — gateway release; :5005 kept for dual-run |
...
| P2 | `apps\Falcon.Net`: `Forms\frmScanTab.cs` (~50 sites), `Forms\frmProduction.cs` (**definition host** — defines `Fire*` methods; the wrappers here → BusAdapter/UiMarshaller/ToolStateReactions are staged), `Forms\frmMain.cs`, `Modules\modWaferAlignment.cs`, `Forms\frmVerifyTab.cs`, `Cmm\CmmReceiverApiRequetsHandler.cs`, `LoginController.cs`, `Classes\clsInitAOI.cs`, `clsMultiRecipe.cs`, `ExternalCoordSystemsAlign.cs`, `clsCalibrationManager.cs`, `MainContextModule.cs`, `Forms\frmJobTab.cs` **(2 live `Fire*` call sites — FEA8-2)** | **~80 `Fire*` call sites across 13 files** (verified count — ~2× the earlier "~40" estimate); dual-publish into FalconWrapper continues | **High file-count, mechanical** — the program's biggest AOI diff; **effort re-priced ~2×** |
| P2 | `apps\Falcon.Net\...\AutoLoaderUIWrapper` (AOI-side republish) | `loader.events` publish from AOI's existing EFEM COM callbacks — **`EfemSrv.exe` (native C++ ATL) is untouched** (it has no bus client; `camtek_bus.dll` is deferred, 06 §6.1), mirroring the ScenarioManager republish pattern (FEA7-1/M-30) | Low — additive, AOI-side |
...
| CI (`xbuild\*.yml` — at the repo root `C:\CamtekGit\xbuild\`, not under `BIS\`; FEA7-5) | **TestKit runs on net48 × both bitnesses** (the build AOI actually loads — not net8-only, R-TS-1) + net8/10. **Tier table (R-TS-2 CI tiers, normative — §6.10 references this):** PR = fast assertions 1–4,6–11 both TFMs (<10 min) **+ gateway suite G-1..G-6**; nightly = 5, 5b, 5c, 6b, 12, 13 + T-L2/3/5 **+ gateway suite G-7..G-9**; release/P0 = T-L1/4/6 + FlaUI + record-replay. Wave-0 exit: the pipeline blocks a deliberately-broken build (today it runs zero tests). *(net48;net8.0 multi-target has no precedent in `BIS\Sources` — a P0 checklist item verifies `Camtek.CSharp.Common.Properties.props` applies to SDK-style multi-target + the .NET 10 SDK is on the CI agents.)* |
| Test instruments (R-TS-2) — **with pass bounds (M-22)** | **GEM record-replay** (built + mutation-qualified: **N ≥ 100** seeded mutations across **byte / order / timing / duplication** classes → 100 % flagged, **0 false alarms over M ≥ 20 clean replays**; N,M finalized at P0) and the **shadow comparator** (100 % of the pre-registered injected-divergence taxonomy flagged **AND unexplained-flag rate ≤ X per 10 000 clean event pairs**, X pre-registered at Wave 0 — a qualification with no bound cannot fail). `TestAutomationAPI` (**exists** in-repo — FlaUI suite, not a build) needs only its CI-wiring task named. Wave-0 exit = both bounds met with evidence |
| **Results-directory retention (M-38/DI7-4)** | Tile/zip data a WAL entry points to must be retained **≥ (max WAL drain window + dead-letter recovery window)** — else a long outage ages out the data under a still-`Pending` WAL leg and the drain false-poisons an entry whose data is gone. A Wave-0 deliverable with a test |
