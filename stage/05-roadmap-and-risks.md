...
| A-6 | Gateway WAL + idempotency (R-3, R-4) | Ack-on-WAL-only, per-sink state machine, backpressure-at-quota, **gateway-side dedup on `(source,epoch,seq)`** so duplicates don't leave the gateway (§6.5, §1.3.2) | Fleet/TSMC `messageId` dedup is now **defense-in-depth** for the residual crash-between-ack-and-mark window — a *recommended* cross-team hardening, no longer a correctness blocker | ✅ design-complete (residual = optional hardening) |
