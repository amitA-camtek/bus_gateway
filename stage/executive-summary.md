...
> **Status:** DESIGN READY (bus core) — **eight adversarial review cycles complete.** The message-bus core is settled and multiply verified. The **7th cycle re-grounded the ToolConnect gateway (doc 07) and the GEM seam** — specification gaps now resolved, three items flagged for owner ratification (D1–D3). The **8th cycle applied all nine lenses in parallel** and found design completeness and operations gaps — all CRITICAL/MAJOR findings resolved in the design ([stage-review-cycle8.md](stage-review-cycle8.md)) — with seven new items for human sign-off (D15–D21), including one architecture decision (GEM session-0 split, D15) and one required P1a cross-team dependency (Fleet dedup key, D7 — **not optional**).
> **Basis:** verified codebase investigation against `C:\CamtekGit\BIS\Sources`, an independent communication census of AOI_Main, and eight adversarial review cycles covering consistency, feasibility-vs-code, operations, concurrency, connectivity, load, security, data-integrity, and test-strategy.
> **Full design chain:** [00-context-and-case.md](00-context-and-case.md) · [01-system-architecture.md](01-system-architecture.md) · [02-aoi-architecture.md](02-aoi-architecture.md) · [03-appendix-four-lanes.md](03-appendix-four-lanes.md) · [04-impact-analysis.md](04-impact-analysis.md) · [05-roadmap-and-risks.md](05-roadmap-and-risks.md) · [06-bus-implementation.md](06-bus-implementation.md) · [07-toolconnect-design.md](07-toolconnect-design.md) · review record in [stage-review.md](stage-review.md).
...
- **Wave 2:** EOL-runtime containment complete (the :50055 loopback surface put behind an authenticated, per-operation-authorized :5007 door — :50055 was already loopback-bound, so this is runtime containment, not the closure of an external surface), process count target met, the gRPC service pattern proven.
...
The design passed **eight adversarial review cycles**. The first four covered consistency, feasibility-vs-code, operations, concurrency, connectivity, and load (thirteen reviewers, all CRITICAL/MAJOR findings resolved). The fifth added **security, data-integrity, and test-strategy** and found ≈8 protocol-level gaps — including a durable-subscriber gap that would have lost data on a gateway restart and a publisher-epoch omission that would have discarded fresh wafers after an AOI restart. The sixth was a full consistency + Mermaid-lint sweep (all 39 diagrams parse; 17 findings landed).

The **seventh cycle pointed all nine lenses at the ToolConnect gateway (doc 07) and the GEM seam, re-grounded in the shipped code.** It found 9 gateway-level specification gaps — all resolved ([stage-review-cycle7.md](stage-review-cycle7.md)); three are flagged for owner ratification (CMM authorization, HCACK-4 accept path, Fleet-egress posture).

The **eighth cycle ran all nine dimensions in full parallel against the complete stage set.** It found 8 unique CRITICALs (including a journal-replay race that would break the durability guarantee on broker restart, a sidecar ordering gap that would silently duplicate wafers on daily restarts, and three operations CRITICALs — SecsGemGui.Net in session 0, legacy AOI killing the supervised gateway, and dual-run mode having zero tests). All 8 CRITICALs and all 28 unique MAJORs are resolved in the design ([stage-review-cycle8.md](stage-review-cycle8.md)); seven new items for human sign-off were added (D15–D21). The feasibility reviewer confirmed all specific code-level citations were exact — a rare quality signal for a design at this scale.

The design is **READY** in the precise sense that every fork is decided, every critical/major finding is resolved, and no design work remains — except one: the SecsGemGui.Net session-0 split (D15, required before Wave 1 GEM entry). What remains is normal program governance: ratify decisions, name owners (an existing pre-P0 entry criterion), take routine P0 measurements, run Wave-0 builds, execute one P3 code spike, close the **required** Fleet dedup-key P1a cross-team dependency (D7 / A-6 — not optional), and let each customer set the one commissioning policy that is theirs to set (the P4+ degraded-mode choice between "refuse Production entry" and "supervised ONLINE-LOCAL override" — a fab safety judgement, not an engineering one).
