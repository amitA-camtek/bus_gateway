...
> **Status:** DESIGN READY (bus core) — seven adversarial review cycles complete. The message-bus core is settled and multiply verified. The **7th cycle re-grounded the ToolConnect gateway (doc 07, authored after the cycle-5 verdict) and the GEM seam in the shipped code and found gateway-level specification gaps** — now resolved in the design ([stage-review-cycle7.md](stage-review-cycle7.md)) — with three items flagged for **owner ratification** (CMM per-operation authorization, the host-visible GEM accept-path change, the Fleet-egress cleartext posture). Design-READY for the bus; the gateway/GEM contracts are specification-complete pending those sign-offs.
> **Basis:** verified codebase investigation against `C:\CamtekGit\BIS\Sources`, an independent communication census of AOI_Main, and seven adversarial review cycles covering consistency, feasibility-vs-code, operations, concurrency, connectivity, load, security, data-integrity, test-strategy, and (7th cycle) the fab-facing GEM/AOI seam — the 6th a full Mermaid-lint consistency sweep, the 7th a code-grounded re-review of the ToolConnect gateway and GEM seam.
...
The design passed **seven adversarial review cycles**. The first four covered consistency, feasibility-vs-code, operations, concurrency, connectivity, and load (thirteen reviewers, all CRITICAL/MAJOR findings resolved). The fifth added **security, data-integrity, and test-strategy** and found ≈8 protocol-level gaps — including a durable-subscriber gap that would have lost data on a gateway restart and a publisher-epoch omission that would have discarded fresh wafers after an AOI restart. The sixth was a full consistency + Mermaid-lint sweep (all 39 diagrams parse; 17 findings landed).

The **seventh cycle pointed all nine lenses at the ToolConnect gateway (doc 07, authored *after* the fifth-cycle "READY" verdict and never multi-dimension-reviewed) and the GEM seam, re-grounded in the shipped code.** It found 9 gateway-level specification gaps the earlier verdict had not tested — the WAL state machine lacked ownership/in-flight/durability contracts (steady-state duplicate + a daily-restart silent-loss path), the poison-vs-outage split keyed on a code flag that misclassifies both ways, the P1a dual-run mode was unspecified, the CMM proxy lacked per-operation authorization, and the two GEM mechanisms (transition ring, async-accept) had no registered topic / no driver primitive. **All are now resolved in the design** ([stage-review-cycle7.md](stage-review-cycle7.md)); three are flagged for owner ratification (CMM authorization, the host-visible HCACK-4 accept path, the Fleet-egress posture). The lesson recorded honestly: doc 07 had been declared READY on the bus core's review depth, not its own — the seventh cycle closed that gap.
