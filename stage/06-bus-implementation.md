...
public interface IBus : IDisposable
{
    ...
    // Commands — untyped reply (ACCEPTED/REJECTED + Reason string, for gui.commands / tool.commands).
    // Ttl mandatory; requester-side deadline mandatory (never wait bare).
    ...
    // The wire envelope carries the serialized TRes as its payload; Reply metadata is synthesized.
    // Serving handler returns TRes directly; the library wraps it in the reply frame. (C9-1)
    ...
    // Broker liveness PING — proves broker answers, not just that the pipe is open.
    // PING is priority-queued at the broker (ahead of PUB traffic). Returns true if
    // a PONG was received within the timeout; false if the broker is silent/hung. (C9-3)
...
- **Reconnect algorithm:** the high-water **H = the max seq released to the pump**, recovered on restart as **`max(seq-sidecar, journal seq)`** for the current epoch — the monotonic sidecar (§6.3) is required because compaction removes acked top entries, so `max(journal seq)` alone can regress H below seqs already delivered downstream, silently dropping the next fresh wafers as duplicates (X7-9). **Sidecar update rule (C8-CRIT-6):** sidecar := max(sidecar, max seq being removed by compaction), **fsynced before `ReplaceFile` begins** (compaction is gated on sidecar durability — not the reverse). A crash mid-compaction with an unfsynced sidecar would let the recovered sidecar regress, making the next fresh wafers look like duplicates on daily restart. Epoch file is bumped and fsynced **before** the new journal file is created (an epoch+empty-journal is safe; an old-epoch+new-journal would regress the dedup baseline). Replay journal strictly in seq order to H → then drain the live queue discarding class-A ≤ H → per-`(source,topic)` FIFO holds; the caller never pauses. Reconnect backoff is jittered; replay paced by **NACK/RESUME + 32-message bounded in-flight window** (the "pacing" is the broker's NACK-queue-full + RESUME-at-low-watermark pair, not a separate credit frame). **Journal replay is a chunked, cooperative job (C8-CRIT-2):** the journal thread processes ≤64 entries per batch, re-posts itself to the back of its job queue, and processes any pending appends/tombstones before the next chunk. A replay job never occupies the journal thread longer than one group-commit interval × batch-size. The credit wait (NACK pause) lives on the pump writer thread, never on the journal thread — so new `scan.committed` publishes accepted during replay remain durable even if the broker crashes mid-replay.
...
classDiagram
    class IBus {
        ...
        +RequestAsync~T~(topic, payload, ttl, ct) Task~Reply~ : untyped reply
        +Serve~T~(topic, handler) ISubscription : untyped reply
        +RequestAsync~TReq,TRes~(topic, payload, ttl, ct) Task~TRes~ : typed reply
        +Serve~TReq,TRes~(topic, handler) ISubscription : typed reply
        +PingAsync(timeout) Task~bool~ : broker liveness probe
        +Health BusHealth
...
- **Test (assertion 5 as extended by R-3):** per-sink `count == published AND distinct-count == published` across {gateway-down-at-PUB, gateway-crash-with-queued-entries, gateway-crash-between-sink-push-and-DELIVER_ACK, publisher-restart} — a plain count is blind to a loss cancelled by a duplicate.
...
- **Class-A queues (C8-MAJ-21):** bounded queue per subscriber. **Config-load invariant: Σ(declared publishers × per-publisher in-flight window) ≤ queue capacity** — or credit pacing is active (RESUME carries per-connection credit = free-slots / NACKed-connections, jittered). Sizing: 6 publishers × 32 in-flight = 192 → **256 slot capacity** (the earlier 128 = "2× worst burst" was sized for burst, not concurrent replay — if all 6 sources replay with 32 in-flight simultaneously, 128 guarantees a NACK storm that stalls the journal thread's replay job, breaking the durability-immediately guarantee). Full → `NACK`; drained → `RESUME`. E2E-ack tracking bounded by Σ queue capacities; publisher disconnect purges its routing entries.
- **Class-B (C8-MAJ-3):** `RetainedSlot` carries **per-(slot, subscriber-connection) dirty state** — a CAS on `(SourceEpoch, StateSeq)` where the **higher pair wins** (`Update` is seq-max, not last-write-wins). The per-connection writer task consumes from its slot cursor (coalescing delivery); the direct-enqueue-into-a-shared-dirty-bit path is removed because one consumer consuming the slot's dirty bit prevents other connections from seeing the update. **Coalesce per (topic, key)**; a per-topic-only slot collapses multi-key state like `production.carrier`; **retained** — last value per (topic, key) delivered to every new subscriber on subscribe. **After a broker restart the in-memory retained slots are empty; every class-B publisher re-publishes its current value on (re)connect** (resolves R-5) — the broker stays persistence-free, and the publisher (which owns the state) restores it with the same `stateSeq`, which dedup absorbs. When serving a retained value the broker attaches `sourceConnected` + `retainedAtUtc`. **Mid-session owner death (M-7/CN7-3):** because `sourceConnected` was only ever attached *at serve time*, a subscriber that connected while the owner was alive would never learn the owner later died (no new serve occurs). So the broker, on detecting a retained-topic owner's disconnect, delivers a **synthetic retained update** (same envelope, `sourceConnected=false`, `disconnectedAtUtc`) to the *current* subscribers of that (topic, key) — still stateless, it already owns the connection fact. A subscriber treats `sourceConnected == false` beyond T as degraded input; the gateway derives `stale-since` from it ([07 §7.5](07-toolconnect-design.md)).
...
classDiagram
    ...
    class BoundedQueue~T~ {
        class A : 256, full = NACK, drain = RESUME
...
Derived: broker class-A queue **256** (config-load invariant: Σ publishers × in-flight window = 6 × 32 = 192 → 256 — see §6.6; the earlier 128 was sized for burst, not concurrent replay); journal 100k/256 MB (alarm 50%; the count-cap binds only under the per-topic payload budget of §6.3 — telemetry ≤ 2 KB); **gateway WAL quota 4 GB ≈ 1.6 M entries (alarm 50/80 %), with a per-topic floor reserving `scan.committed`'s share** ([07 §7.5](07-toolconnect-design.md)); **1-h outage backlog model = ≤4,000 entries** (1 h of max-nominal per-wafer traffic at 60 wph; telemetry storms excluded — separate storm-coincident variant with its own bound gated on 60 msg/s FleetSink ceiling); gateway channel 1000 (~17–33 min of **nominal** traffic — outage absorption is the WAL's job, drained at the rate cap T-L4 asserts: a 1-h outage backlog clears < 10 min); dedup OOO window 64; replay in-flight window 32 (**invariant: in-flight window ≤ OOO window, asserted at config load**). **Storm fan-in: 10/s × N sources (N ≈ 6) = 60 msg/s aggregate per tool** — FleetSink's P0 ceiling must be ≥ 60 msg/s (or gateway egress coalesces). P0 publishes measured **single-instance ceilings** (broker msg/s + MB/s at p99, batch-fsync/s under co-load, **FleetSink msg/s — acceptance ≥ max(60 storm, 7 T-L4-drain)**, **TsmcSink service time — acceptance ≤ 8.5 s/wafer (≥ 420/h) or T-L4 is re-scoped per sink**, **`scan.dds-node-status` emit rate — must be measured before it is sized/registered**) — scale-out is a documented non-requirement with an expiry condition. The future `dds.frame.*` tier carries **no normative number here** — sizing is deferred to the DDS census. Fleet-side herd control: jittered registration + drain start (0–120 s) + per-tool drain caps.
...
| 12 | Broker restart under load, **all declared publishers of the topic (≥6)** with journals → convergence, no NACK oscillation. Config-load invariant check: Σ(publishers × in-flight window) ≤ queue capacity must be asserted at startup (6 × 32 = 192 ≤ 256). Test also exercises the credit/pacing path: overdrive in-flight window above the per-publisher credit budget → NACK fires → journal queues + redelivers on RESUME with zero loss; nightly-sized journal fill (C8-MAJ-21) |
...
**Usage flow (a consumer contract test):** `BusHarness` up → subscribe the component under test → publish via the harness → inject the scripted fault at the scripted step → `TopicCaptor` asserts delivery/order/dedup/drop counters → harness restart → captor asserts replay semantics. The harness is also the fixture the assertion table runs against in CI. **CI tiers: see [04-impact-analysis.md §4.4](04-impact-analysis.md) (normative single source of truth).** The table here is omitted to prevent drift — 04 §4.4 governs. **Net48 nightly invariant (C8-MAJ-28):** nightly composites run the net48 client (x86 and x64) against the net8 broker; the net8 client variant additionally runs for broker-side assertions. **Net48-nightly-green is a P1a entry criterion.** Add assertion 5c (WAL I/O failure) and the gateway suite (G-1..G-10) to the nightly tier per 04 §4.4.
