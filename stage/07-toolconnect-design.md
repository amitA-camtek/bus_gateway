...
2. **Exactly one dispatch authority per (entry, sink) (X7-1/CC7-1).** A leg is dispatched only via an atomic `Pending → InFlight(owner, leaseUtc)` transition through the actor; `NextPending` never returns an `InFlight` leg, so a drain tick can never re-send a leg a live send is currently transmitting. Completion is `InFlight → Done`; failure is `InFlight → Pending|Poison`; a **lease timeout** (> the sink deadline) reclaims a leg orphaned by a crashed send. This closes the steady-state (no-crash) duplicate.
...
- **Quota sizing (M-34).** Quota = **4 GB ≈ 1.6 M entries at ~2.5 KB avg → ≈18 h of sustained burst (25/s) or ≈18–37 days of nominal (0.5–1/s); alarm at 50 % and 80 %.** (The earlier "a full hour of burst = < 1 % of quota" was arithmetically false for any quota below ~22.5 GB — 1 h of sustained burst is ~225 MB ≈ 5.5 % of 4 GB.)
...
- **Health = consumption-liveness token** pushed to ToolHost: "connected AND processed-or-idle within T" — a hung pipeline is distinguishable from an idle one; the token also carries the `wal-io-failure`/`wal-quota` bits so gateway health reaches Fleet **off the sink path** (§7.11).
...
| WAL state | **A single WAL-state actor** owns all entry-file mutations and the in-memory index (§7.4 rule 1). `MarkSinkDone`, dispatch-lease, attempt-increment, poison, delete and the quota gauge are messages to this actor, serialized by construction — this replaces the earlier "thread-safe, keyed by entryId+sink" assertion that had no mechanism |
...
| Drain scheduler | Single background loop; **oldest-first, rate-capped, interleaved with a weighted live:drain quantum** (e.g. 4:1, work-conserving — mirrors the S-14 send-queue fix), **not** "live wins ties" (which gives drain no guaranteed share and lets sustained live traffic starve the drain → a past outage becomes future refuse-new). T-L4 (1-h outage drains < 10 min) is asserted **with concurrent live traffic** and is gated by the P0 sink-throughput floors (§6.10: FleetSink ≥ 7 msg/s; TsmcSink ≤ 8.5 s/wafer) — if a P0 measurement misses its floor, T-L4 is re-scoped per sink at Wave 0, not discovered impossible later |
...
- **Shadow-door WAL entries are marked `ShadowDone`** on comparator consumption — a quota-exempt audit class, so shadow traffic never counts toward the quota that gates live backpressure.
- **The P1b flip procedure** names the old-`FailedMessages`-spool vs WAL overlap: at flip, a message present in both stores is suppressed by `UniqueId` for N minutes so the retention hand-off cannot double-submit.
...
| G-8 | Crash-point matrix (the four `CrashAt` points) + compact-to-empty → restart → **fresh wafer accepted, not deduped** (X7-3/X7-9); pairs with TestKit 5/5b | nightly |
| G-9 | Scaled drain (~5 k mixed-state entries): oldest-first, rate cap honored, no re-send to `Done` legs, live-traffic latency bound during drain (cheap proxy for T-L4) | nightly |

## 7.13 Alarm routing (normative — resolves M-16/OPS7-1)

Every gateway/WAL/journal alarm has a stated surface and operator action; critically, **gateway/WAL/journal alarms reach Fleet via the ToolHost `:5100` health push** (counters survive broker death, [06 §6.6](06-bus-implementation.md)) — **never solely via `tool.telemetry`**, which is class-A through the same WAL the alarm is about and would be backpressured by the very condition it reports.

| Alarm | Local surface | Fleet surface | Operator action |
|---|---|---|---|
| WAL 50 % / 80 % quota | operator banner + light tower | :5100 push | check Fleet/TSMC connectivity; drain is automatic on recovery |
| WAL quota reached (withhold-ACK) | banner + light tower | :5100 push | Fleet/TSMC outage — data is durable, not lost; escalate if prolonged |
| `wal-io-failure` (disk error/full) | banner + light tower | :5100 push | **FSE**: free disk / check the WAL volume |
| Poison dead-letter | banner | :5100 push + count | inspect/re-inject via the dead-letter CLI (§7.10 deliverable) once the poison cause is fixed |
| Epoch new / **regression** | log + banner (regression) | :5100 push | regression on a rollback is expected; otherwise investigate a journal clone/tamper |
